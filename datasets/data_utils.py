...
class PointcloudRotateByDegree(object):
    def __init__(self, axis=np.array([0.0, 1.0, 0.0]), angles=np.array([0, 0, 0]), p=1):
        self.axis = axis
        self.p = p
        self.angles = angles / 360 * 2 * np.pi
    ...
    def __call__(self, points):
        ...
        if self.axis is None:
            ...
            rotation_matrix = torch.matmul(torch.matmul(Ry, Rz), Rx)
        else:
            #rotation_angle = np.random.uniform() * 2 * np.pi
            rotation_matrix = angle_axis(self.angles, self.axis)
...
class PointcloudRandomInputDropout(object):
    ...
    def __call__(self, points):
        ...
        dropout_ratio = np.random.random() * self.max_dropout_ratio  # 0~0.5
...
class PointcloudScaleAndTranslate(object):
    ...
    def __call__(self, pc):
        ...
        pc = torch.mul(pc, torch.from_numpy(xyz1).float()) + torch.from_numpy(xyz2).float()
